    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('One-time Expenses') }}
            </h2>
            <a href="{{ route('admin.payroll.onetime-expenses.create') }}" class="btn btn-primary">
            ...
            <div class="card-header bg-primary text-white py-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0 text-white">
                        <i class="bx bx-money me-2"></i> One-time Expenses
                    </h5>
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('admin.payroll.onetime-expenses.create') }}"
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-hover">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-select form-select-sm payment-status-select"
                                            ...
                                            data-original-status="{{ $expense->payment_status }}">
                                            @foreach (App\Models\OnetimeExpens::getPaymentStatuses() as $value => $label)
                                                <option value="{{ $value }}"
                                                    {{ $expense->payment_status === $value ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            ...
                                            <a href="{{ route('admin.payroll.onetime-expenses.edit', $expense->id) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ Auth::user()->role === 'super-admin' ? 7 : 6 }}"
                                        class="text-center">
                                        No expenses found.
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="importModalLabel">Import One-time Expenses</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="importForm" action="{{ route('admin.payroll.onetime-expenses.import') }}" method="POST"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="alert alert-info">
                            ...
                            <ul class="mb-0">
                                ...
                                <li>payment_status (optional - values: pending, paid, cancelled)</li>
                            </ul>
                            <p class="mt-2 mb-0"><small>Notes:</small></p>
                            <ul class="mb-0">
                                <li><small>Date can be in Excel date format or text format (YYYY-MM-DD)</small></li>
                                <li><small>Amount can include currency symbols and thousand separators (e.g.,
                                        $2,500.00)</small></li>
                                <li><small>If payment_status is not provided, it will default to 'pending'</small></li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            <div class="card">
                                <div class="card-body">
                                    <h6 class="card-title">Rows with Errors</h6>
                                    <p class="card-text" id="errorRows">0</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                    ...
                    button.addEventListener('click', function() {
                        ...
                        showDeleteConfirmation({
                            ...
                            if (result.isConfirmed) {
                                ...
                                form.method = 'POST';
                                form.action =
                                    '{{ route('admin.payroll.onetime-expenses.delete', '') }}/' +
                                    expenseId;
                ...
                previewBtn.addEventListener('click', async function() {
                    ...
                    try {
                        ...
                        const response = await fetch(
                            '{{ route('admin.payroll.onetime-expenses.preview') }}', {
                                method: 'POST',
                                body: formData,
                                headers: {
                ...
                confirmImportBtn.addEventListener('click', async function() {
                    ...
                    try {
                        ...
                        // Send import request
                        const response = await fetch(
                            '{{ route('admin.payroll.onetime-expenses.import') }}', {
                                method: 'POST',
                                body: formData,
                                headers: {
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                ...
                function updatePreviewModal(preview) {
                    ...
                    previewData.forEach(row => {
                        ...
                        <td>${escapeHtml(data.payment_status || 'pending')}</td>
                        <td>
                ...
                document.querySelectorAll('.payment-status-select').forEach(select => {
                    select.addEventListener('change', async function() {
                        ...
                        try {
                            const response = await fetch(
                                `/admin/onetime-expenses/${expenseId}/update-status`, {
                                    method: 'POST',
                                    headers: {
                            ...
                            if (data.success) {
                                showSuccessAlert('Payment status updated successfully');
                            ...
                            showErrorAlert(error.message || 'Error updating payment status');
